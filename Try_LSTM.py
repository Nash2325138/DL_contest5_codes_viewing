...
parser.add_argument('--pipe-r', type=float, default=0.2, help='Reward of passing each pipe')
parser.add_argument('--alive-r', type=float, default=0.01, help='Reward of keep alive for t')
    ...
    def build_model(self):
        ...
        def net(screen, reuse=False):
            with tf.variable_scope(
                ...
                screen_list = tf.split(screen, num_stack, 3)
                CNN_outputs = []
                for one_screen in screen_list:
                    with tf.variable_scope('CNN', reuse=tf.AUTO_REUSE):
                        ...
                        pool1 = tf.layers.max_pooling2d(
                            conv1, pool_size=[2, 2], strides=[2, 2], padding='SAME', name='pool1')
                        ...
                        flat = tf.layers.flatten(conv3, name='flat')
                        CNN_outputs.append(flat)
                CNN_outputs = tf.stack(CNN_outputs, 1)
                print(CNN_outputs.get_shape())
                    
                rnn_hidden_size = np.prod(CNN_outputs.get_shape().as_list()[2:])
                cell = tf.contrib.rnn.BasicLSTMCell(num_units=rnn_hidden_size, state_is_tuple=True)
                print('cell:', cell)
                zero_state = cell.zero_state(tf.shape(screen)[0], tf.float32)
                self.rnn_outputs, final_state = tf.nn.dynamic_rnn(
                    inputs=CNN_outputs,
                    cell=cell,
                    dtype=tf.float32,
                    initial_state=zero_state
                )
                print('rnn_outputs:', self.rnn_outputs)
                print('final_state:', final_state)
                
                flat = self.rnn_outputs[:, -1, :]
                print('flat:', flat)
...
ts = []
while len(buffer.experiences) < buffer.buffer_size * 0.2:
