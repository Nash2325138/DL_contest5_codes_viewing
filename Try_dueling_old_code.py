...
class Agent:
    ...
    def update_parameters(self, episode):
        next_epsilon = self.init_epsilon - \
                       (self.init_epsilon - MIN_EXPLORING_RATE) * episode / 3000000
...
for episode in range(restore_episode, restore_episode + NUM_EPISODE + 1):
    ...
    while not env.game_over():
        ...
        if episode > restore_episode + NUM_EXPLORE:
        ...
        # update agent
    if t > best_t and episode > restore_episode + NUM_EXPLORE:
        best_t = t
...
#         clip.write_videofile(os.path.join(movie_dir, "DQN-{}_BSF.webm".format(episode)), fps=60)
        
    if episode % update_every_episode == 0 and episode > restore_episode + NUM_EXPLORE:
        update_target(update_ops, sess)
    ...
    if episode % print_every_episode == 0 and episode > restore_episode + NUM_EXPLORE:
        print("[{}] time live:{}, cumulated reward: {:.3f},\
        online/target exploring rate: {:.6f}/{:.6f}, loss: {:.6f}".format(
            episode, t, cum_reward, online_agent.exploring_rate, target_agent.exploring_rate, loss))
    ...
    if episode % ckpt_every == 0 and episode > restore_episode + NUM_EXPLORE:
